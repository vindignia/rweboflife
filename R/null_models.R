...
swap_model = function(M_in, iter_max) {
  ...
  if (is.null(iter_max)) {
    ...
    warning(
      paste0(
        "as the parameter iter_max was not assigned, the default value iter_max = ",
        iter_max,
        " was assumed"
      )
    )
...
#' @NoRd
#' @import permute
curveball_model = function(M_in, iter_max) {

  # this algorithm is described in the manuscript
  # Strona, G., et al.: A fast and unbiased procedure to randomize ecological binary matrices with fixed row and column totals.
  # Nature Commun. vol. 5, 4114 (2014). DOI 10.1038/ncomms5114
  # The present function is adapted from the code provided by the authors along with the manuscript

  nr <- nrow(M_in) # number of rows
  nc <- ncol(M_in) # number of columns

  # binarize M_in into a logical matrix
  M_in <- as.matrix((M_in > 0))
  class(M_in) <- "numeric"

  # locate the elements equal to 1 in each row, and store them in a list
  M = if (nc >= nr) M_in else t(M_in)
  hp = list()
  for (row in 1:nrow(M)) {
  ...
  M_res = matrix(0, nrow(M), ncol(M))
  for (row in 1:nrow(M)) {
    M_res[row, hp[[row]]] = 1
  }

  M_curveball = if (nc >= nr) M_res else t(M_res)
  return(M_curveball)
}


#' @export
null_model <- function(M_in,
                       iter_max = NULL,
                       model = NULL) {
  ...
  if (is.null(model)) {
    stop(
      "No type argument assigned for the null model. Please choose one from the list: \"swap\", \"equifrequent\", \"cell\", \"curveball\""
    )
  } else {
    if (model == "swap") {
      M_out <- swap_model(M_in, iter_max)
    } else if (model == "curveball") {
      M_out <- curveball_model(M_in, iter_max)
