test_that("check that the number of 1 values is preserved in the equifrequent null model", {
  ...
  # the swap and curveball models fulfill the more stringent condition of preserving the row and column totals => checked later on
...
test_that("check that the row and columns totals are preserved in the curveball null model", {
  ...
  M_curveball <- null_model(M, iter_max = ceiling(3*sqrt(nrow(M)*ncol(M))), model = "curveball")

  # check row total
  rows_tot_rnd <- c()
  rows_tot_original <- c()
  for(i in seq(1,nrow(M))){
    rows_tot_rnd <- append(rows_tot_rnd, sum(M_curveball[i,]))
    rows_tot_original <- append(rows_tot_original, sum(M_curveball[i,]))
  }
  expect_true(all(rows_tot_rnd == rows_tot_original))
  ...
  cols_tot_original <- c()
  for(j in seq(1,ncol(M))){
    cols_tot_rnd <- append(cols_tot_rnd, sum(M_curveball[,j]))
    cols_tot_original <- append(cols_tot_original, sum(M_curveball[,j]))
  }
  expect_true(all(cols_tot_rnd == cols_tot_original))
